...
const printPizzaFansVTwo = (friends, pizzaOffers) => {
  return friends.map(friend => {
    const preferredPizzas = pizzaOffers.filter(pizza => {
      if (
        friend.preferences.some(pref => pizza.toppings.includes(pref)) ===
          true &&
...
const printPizzaFansVThree = (friends, pizzaOffers) => {
  return friends.map(friend => {
    ...
    const mostlyMatchedPrefCounter = preferredPizzas
      .map(pizza => +pizza.matchCounter)
      .sort((a, b) => b - a)[0];
...
const printFriendsForAPizza = (pizza, friends) => {
  ...
  const highestFriendPrefMatches = Object.values(friendPizzaDictionary).sort(
    (a, b) => b - a
  )[0];
...
const printFriendsForAPizzaVersionTwo = (pizza, friends) => {
  ...
  const highestFriendPrefMatches = +friendPizzaObject
    .map(obj => obj.matchCounter)
    .sort((a, b) => b - a)[0];

  // Filtering out the friends whose matchCounter is the highest
  const friendsToGetThePizza = friendPizzaObject
    .filter(
      friendPizza => +friendPizza.matchCounter === +highestFriendPrefMatches
    )
...
function digital_root(n) {
  return n > 9
    ? digital_root([...n.toString()].reduce((acc, num) => (acc += +num), 0))
    : n;
}
...
function iqTest(numbers) {
  ...
  numbers
    ...
    .forEach((elem, index) =>
      +elem % 2 !== 0
        ? dictionary.odd.push({ elem, index })
        : dictionary.even.push({ elem, index })
    );
  return dictionary.odd.length === 1
    ? dictionary.odd[0].index + 1
    : dictionary.even[0].index + 1;
}
...
const moveZeros = function(arr) {
  const arrayOfZeros = arr.reduce(
    (acc, elem) => (elem === 0 ? [...acc, elem] : acc),
    []
  );
...
const validSolution = board => {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    if (i === endIndex - 1) {
      if (
        checkBlockValidity(blockOfNine) === true &&
        endIndex >= board[0].length == false
      ) {
        startIndex += 3;
        endIndex += 3;
      } else if (
        checkBlockValidity(blockOfNine) === true &&
        endIndex >= board[0].length == true
      ) {
        isValid = true;
      } else {
        isValid = false;
...
const uniqueInOrder = iterable => {
  ...
  const filteredIterable = iterable
    .split("")
    ...
    .reduce((acc, num) => {
      return [
        ...acc,
        {
          ...
          weight: num
            .split("")
            .reduce((acc, singleNum) => (acc += +singleNum), 0)
        }
      ];
    }, [])
...
function topThreeWords(text) {
  ...
  const threeBiggest =
    counterArray.length > 3 ? counterArray.slice(0, 3) : counterArray;

  const threeMostOftenElements = threeBiggest
    .map(num => {
...
const replaceWithSynonyms = str => {
  return str
    .split(" ")
    .map(word => (wordsAndSyn[word] ? wordsAndSyn[word] : word))
    .join(" ");
};
...
// into: 
// const list = [
// 	{key: "a", count: 2, values: ["1", "2"]},
// 	{key: "b", count: 1, values: ["1"]},
// 	{key: "c", count: 2, values: ["1", "2"]},
// ]
...
const transformArray = list => {
	...
	list.forEach((item) => {
		...
		ownList.some(newObject => newObject.key === key) ? ownList.find(newObject => newObject.key === key).values.push(value) : ownList.push({key, values: [value]})
	})

	return ownList.map(item => {
		return {
			key: item.key,
...
console.log(transformArray(originalArray));
