...
function sortArray(array) {
    ...
    const indexOfEvenNumbers = array.reduce((acc, num, index) => num % 2 === 0 ? [...acc, {[num]: index}] : acc ,[]);
...
function createPhoneNumber(numbers){
  ...
  function duplicateCount(text){
    ...
    const counterObject = toBeCheckedText.map(letter => {
        return{
            ...letter,
  ...
  function iqTest(numbers){
    ...
    numbers.split(" ").forEach((elem, index) => +elem % 2 !== 0 ? dictionary.odd.push({elem, index}) : dictionary.even.push({elem, index}))
